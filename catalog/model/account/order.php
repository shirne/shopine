...
class ModelAccountOrder extends Model {
	...
	public function getOrders($start = 0, $limit = 20, $filters = null) {
		if ($start < 0) {
			$start = 0;
		}
		$wherestr='';
	...
	public function getTotalOrders($filters=null) {
		$wherestr='';
		if(is_array($filters)){
			if(isset($filters['start_date'])){
				$wherestr .= ' AND date_added>=\''.$filters['start_date'].'\' ';
			}
			if(isset($filters['end_date'])){
				$wherestr .= ' AND date_added<=\''.$filters['start_date'].'\' ';
			}
			if(isset($filters['order_id'])){
				$wherestr .= ' AND order_id LIKE \'%'.$filters['order_id'].'%\' ';
			}
			if(isset($filters['order_status'])){
				$wherestr .= ' AND order_status_id=\''.$filters['order_status'].'\' ';
			}
		}else{
			$wherestr .= 'AND order_status_id > \'0\'';
		}
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$this->customer->getId() . "' $wherestr");
