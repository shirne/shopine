...
class ModelCatalogProduct extends Model {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				...
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox') {
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_value = '" . $this->db->escape($product_option['option_value']) . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'");
				...
				}elseif($product_option['type']=='union_option'){
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET  product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'");
                    ...
                    if (isset($product_option['product_option_value'])) {
                        foreach ($product_option['product_option_value'] as $product_option_value) {
                            if(empty($product_option_value['product_value']))
                                $product_option_value['product_value']='';
                            else
                                $product_option_value['product_value']=$this->db->escape($this->joinUnionValue($product_option_value['product_value']));
                            $this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET  product_value= '" . $product_option_value['product_value'] . "', product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "', product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . $this->db->escape($product_option_value['option_value_id']) . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "',  points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "'");
                        }
                    }
				}else {
	...
	public function editProduct($product_id, $data) {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				if(!isset($product_option['option_value']))$product_option['option_value']='';
				if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'text') {
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET  product_id = '" . (int)$product_id . "', option_value = '" . $this->db->escape($product_option['option_value']) . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'");
				...
				}else if($product_option['type'] == 'union_option' ){
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET  product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "'");
	...
	public function getProductOptions($product_id) {
		...
		foreach ($product_option_query->rows as $product_option) {
			if (in_array($product_option['type'],array('select','radio','text','checkbox','virtual_product','color','union_option'))) {
				$product_option_value_data = array();
				
				$product_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND ovd.language_id = '" . ($product_option['type']=='union_option'?'0':(int)$this->config->get('config_language_id')) . "' ORDER BY ov.sort_order ASC, pov.product_option_value_id ASC");
				...
				foreach ($product_option_value_query->rows as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'product_value'           => ($product_option['type']=='union_option')?
                                $this->splitUnionValue($product_option_value['product_value']):
                                $product_option_value['product_value'],
	...
	public function getProductInitOptions(){
		$option_data=array();
		$option_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE o.fixed = 1 AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.sort_order ASC");
		foreach ($option_query->rows as $option) {
			if (in_array($option['type'], array('select', 'radio', 'checkbox', 'virtual_product', 'color'))) {
				$option_data[]=array(
					'product_option_id'    => '',
	...
	private function splitUnionValue($str){
        $arr=array();
        $a1=explode(';',$str);
        foreach($a1 as $a){
            $v=explode(':',$a);
    ...
    private function joinUnionValue($arr){
        $str='';
        foreach($arr as $k=>$a){
            $str .= $k.':'.$a.';';
        }
