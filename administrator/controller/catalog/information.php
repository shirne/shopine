...
class ControllerCatalogInformation extends Controller { 
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$url = $this->url->buildQuery($this->request->get, 'filter_category_id,sort,order,page');
	...
	private function getList() {
		...
		if(isset($filter_category_id)){
			...
			$information_total = $this->model_catalog_information->getTotalInformations($filter_category_id);
		}else{
    	...
    	foreach ($results as $result) {
			...
			$this->data['informations'][] = array(
				...
				'title'          => $result['title'],
				'name'			 => empty($result['name'])?$this->data['text_default']:$result['name'],
		...
		$url = $this->url->buildQuery($this->request->get,'filter_category_id,page');
		...
		$this->data['sort_category'] = $this->url->link('catalog/information', 'token=' . $this->session->data['token'] . '&sort=ic.sort_order' . $url, 'SSL');
		...
		$this->data['categorybase']=$this->url->link('catalog/information', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		
		$url = $this->url->buildQuery($this->request->get, 'filter_category_id,sort,order');
		...
		$this->data['categories'] = $this->model_catalog_information_category->getCategories(0);

		if(isset($filter_category_id))$this->data['filter_category_id'] = $filter_category_id;
		$this->data['sort'] = $sort;
	...
	private function getForm() {
		...
		if (isset($this->request->post['category_id'])) {
			...
			$this->data['category_id'] = $information_info['information_category_id'];
		} elseif (isset($this->request->get['filter_category_id'])) {
			$this->data['category_id'] = $this->request->get['filter_category_id'];
		} else {
