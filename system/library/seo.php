...
final class SEO {
	...
	public function generateSEOURL () {
         ...
         $this->generateInformationCategories();
    ...
    public function generateArticleCategories () {
        $categories = $this->getCategories('article_');
		 foreach ($categories as $category) {
			$uniqueSlug = $this->makeSlugs($category['name']);
           	$query_url = 'article_category_id=' . (int)$category['article_category_id'] ;
            $this->delete($query_url);
            $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = '".$query_url."', keyword = '" . $this->db->escape($uniqueSlug) . "'");
        }
    }
    ...
    public function generateArticles () {
		...
		foreach ($articles as $article) {
            $uniqueSlug = $this->makeSlugs($article['title']);
         	$query_url = 'article_id=' . (int)$article['article_id'] ;
            $this->delete($query_url);
            $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = '".$query_url."', keyword = '" . $this->db->escape($uniqueSlug) . "'");
        }
    ...
    public function generateInformationCategories () {
        $categories = $this->getCategories('information_');
		 foreach ($categories as $category) {
			$uniqueSlug = $this->makeSlugs($category['name']);
           	$query_url = 'information_category_id=' . (int)$category['information_category_id'] ;
            $this->delete($query_url);
            $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = '".$query_url."', keyword = '" . $this->db->escape($uniqueSlug) . "'");
        }
    }
    ...
    private function getCategories ($pre='') {
        $query = $this->db->query("SELECT c.category_id, cd.name,c.parent_id FROM " . DB_PREFIX . $pre . "category c LEFT JOIN " . DB_PREFIX . $pre . "category_description cd ON (c.category_id = cd.category_id) WHERE cd.language_id = '" . (int) $this->config->get('config_language_id') . "' ORDER BY c.sort_order, cd.name ASC");
        return $query->rows;
    }
    ...
    private function getArticles () {
        $query = $this->db->query("SELECT a.article_id, a.title FROM " . DB_PREFIX . "article a WHERE a.language_id = '" . (int) $this->config->get('config_language_id') . "' ORDER BY a.article_id ASC");
        return $query->rows;
    }
